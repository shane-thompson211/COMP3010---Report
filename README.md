...
The main goal of this investigation is to utilise Splunk Enterprise, a Security Information and Event Management (SIEM) platform, to ingest, index, and analyse security logs in order to identify what components of the environment have been compromised. A secondary objective is to determine how the attack occurred and to provide recommendations that would strengthen Frothly’s security posture and prevent future breaches.

The scope of this report focuses on AWS CloudTrail logs, S3 access logs, endpoint logs, and the Windows source type. These data sources are analysed within the BOTSv3 dataset to identify IAM anomalies, rogue devices, multi-factor authentication (MFA) violations, and altered access controls. The analysis is limited to the selected BOTSv3 200-level guided questions and does not attempt to reconstruct the full attack chain. It is assumed that all analysis was conducted within a fully isolated virtual machine to maintain separation from the host system and protect its integrity [3].
...
**Answer:**
- E5-2676
- Full CPU - Intel(R) Xeon(R) CPU E5-2676 v3 @ 2.40GHz
